...
const Popular = () => {
  ...
  const reversedScrollContainerRef = useRef<HTMLDivElement>(null)
  ...
  // Second useEffect for reversed scrolling container
  useEffect(() => {
    const container = reversedScrollContainerRef.current
    if (!container) return
    ...
    // Function to handle seamless loop reset for reversed scroll
    const handleSeamlessLoop = () => {
      if (!container) return
    ...
    // Auto-scroll function for reversed direction
    const autoScroll = () => {
      if (!isUserInteractingRef && container) {
        container.scrollLeft -= 0.5 // Reverse scroll speed (negative)
        handleSeamlessLoop()
        animationId = requestAnimationFrame(autoScroll)
      }
    }
  ...
  return (
    <section id="popular" className="py-16 px-4 bg-white overflow-hidden">
        ...
        >
          {duplicatedTemplates.map((template, index) => (
            <div 
              ...
              className="flex-shrink-0 mx-4 md:mx-6 group cursor-pointer bg-white rounded-2xl shadow-lg hover:shadow-xl transition-all duration-300 overflow-hidden"
                ...
                <div className="text-center">
                  ...
                  {template.discount && (
                    <div className="text-sm text-gray-500 line-through">
                      {Math.round(template.price / (1 - template.discount / 100)).toLocaleString()} DA
                    </div>
                  )}
                </div>
              </div>
      ...
      {/* Desktop: Second line of cards scrolling right to left */}
      <div className="relative w-full hidden md:block mt-8">
        <div 
          ref={reversedScrollContainerRef}
          className="flex overflow-x-auto scrollbar-hide cursor-grab select-none"
          style={{ 
            scrollbarWidth: 'none', 
            msOverflowStyle: 'none',
        ...
        >
          {duplicatedTemplates.map((template, index) => (
            <div 
              key={`reversed-${template.id}-${index}`} 
              className="flex-shrink-0 mx-4 md:mx-6 group cursor-pointer bg-white rounded-2xl shadow-lg hover:shadow-xl transition-all duration-300 overflow-hidden"
              onClick={() => navigate(`/template/${template.id}`)}
            >
              {/* Image */}
